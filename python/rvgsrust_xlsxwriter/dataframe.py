...
`column_formats` (dict[str, Format], keyed by column name) is passed
straight through to write_dataframe(), which merges each column's
requested format into every cell of that column as it's written --
including layering it under the automatic date/datetime number format,
so a border survives on a date column instead of losing to the cell's
own numFmtId under OOXML precedence rules. That merge happens once per
column before the fast path's row loop starts, not per cell, so passing
column_formats does not cost anything on the per-cell hot path. See
Worksheet.write_dataframe()'s docstring in _core.pyi for the merge
details.
...
speed benefit in that case. column_formats works the same way in the
fallback: applied per cell via the same dict lookup, so behaviour is
identical whichever path a given DataFrame takes.
...
either path has written a row, so it is not caught here and does not
trigger a fallback -- the per-cell path would fail the same way for the
same reason.
"""
...
def _write_dataframe_per_cell(worksheet, columns, rows, row, col, header_format, column_formats):
    ...
    column_formats is applied per cell here directly, matching what the
    fast path's Rust-side merge does.
    """
...
def write_polars_dataframe(worksheet, df, row=0, col=0, header_format=None, column_formats=None):
    ...
    Args:
        ...
        column_formats: Dict mapping column names to Format objects.
            Merged per cell by write_dataframe() itself -- see module
            docstring. Raises ValueError (not caught here) for any key
            not present in df.columns.
    """
    if not HAS_POLARS:
    ...
    try:
        ...
        # since retrying with the per-cell path would fail identically.
        pass
