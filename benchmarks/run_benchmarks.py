...
def main() -> None:
    ...
    print()
    print("  rvgs write_dataframe()     : zero-copy Arrow C-stream — fastest path.")
    print("                               pandas/polars/pyarrow all use this route.")
    print("  rvgs write_records()       : 1 FFI call, dict path (hash lookup/cell).")
    print("  rvgs write_rows()          : 1 FFI call, list-of-lists path.")
    print("                               Comparable to write_records(); use for")
    print("                               positional data (CSV readers, DB cursors).")
    print("  rvgs const-mem             : streams rows to disk — lower peak RAM,")
    print("                               but no faster than normal mode.")
    print("  rvgs per-cell              : one FFI call per cell — intentionally slow.")
    print()
    print("  Benchmarked on Intel i5-6267U (2-core, thermal throttling).")
    print("  High stdev is expected — results on Apple Silicon will be tighter.")
    print()
